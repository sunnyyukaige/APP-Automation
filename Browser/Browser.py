...
class Browser(Find):
    ...
    def close_App(self):
        self.__web_driver.close_app()
        self.__web_driver.quit()

    def switch_to_web(self):

        view = self.__web_driver.contexts
        self.__web_driver._switch_to.context(view[-1])

    def switch_back(self):
        self.__web_driver._switch_to.context(None)

    def sent_key_event(self,number):
        self.__web_driver.keyevent(number)
